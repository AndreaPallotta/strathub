...
class PolymarketNFLArbStrategy(BaseStrategy):
    def __init__(self, params=None):
        super().__init__("polymarket_nfl_arb", params)
        ...
        if ref_implied is not None:
            ...
            if edge >= min_spread:
                return {
                    ...
                    "size": max_size,
                    "price": ask,
                    "reason": f"NFL Arbitrage Edge (+{edge:.3f}) >= Min Edge (+{min_spread:.3f})"
                }

        return None

strategy = PolymarketNFLArbStrategy()
